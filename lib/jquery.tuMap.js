...
(function() {
	var f = {
					...
					case "TicketsFilter":
						...
						if (typeof(f.Settings.TicketsFilter.Quantity) == "string") {
							...
							if (i != -1) {
								...
								var m = jQuery.trim(f.Settings.TicketsFilter.Quantity.replace(n, ""));
								if (m == "+") {
									k = "gt"
								}
							}
		...
		RemoveMapControl: function(g) {
			jQuery(".tuMapControl").each(function() {
				if (jQuery.trim(jQuery(this).html()).toLowerCase() == g.toLowerCase()) {
					jQuery(this).remove();
		...
		GetSelectedSections: function(h) {
			var g = [];
			jQuery.each(f.Selection, function(m, j) {
				var l = f.Sections[j];
				var i = null;
				if (l) {
					if (h) {
		...
		GetSections: function(h) {
			var g = [];
			jQuery.each(f.Sections, function(j, i) {
		...
		GetOtherSections: function() {
			var g = [];
			jQuery.each(f.OtherSections, function(i, h) {
				g.push({
					Name: h.Name,
					Alias: h.Alias,
					Enabled: h.Enabled,
		...
		GetSectionsInGroup: function(h, i, g) {
			jQuery.each(f.Sections, function(j, k) {
				if (k == null) {
					return
				}
				if (k && k.GroupId == h) {
		...
		Blink: function() {
			...
			if (f.BlinkState == 1) {
				...
				f.BlinkState = 0;
				jQuery(".tuMapControlSelected").addClass("tuMapControlBlink").removeClass("tuMapControlSelected")
			} else {
				f.BlinkState = 1;
				jQuery(".tuMapControlBlink").addClass("tuMapControlSelected").removeClass("tuMapControlBlink")
			}
			jQuery.each(f.Selection, function(j, h) {
				var i = f.Sections[h];
				if (i != null) {
					i.SetStyle(g)
				}
		...
		FilterTickets: function(g, h, k, j) {
			if (!k) {
				if (f.Selection.length > 0) {
					jQuery.each(f.Selection, function(l, m) {
						if (f.Sections[m]) {
			...
			if (h) {
				...
				var i = -1;
				while ((i = jQuery.inArray(g, f.Selection)) > -1) {
		...
		ToggleAll: function() {
			jQuery.each(f.Sections, function(h, g) {
		...
		ProcessTickets: function() {
			...
			jQuery.each(f.Sections, function(m, n) {
				n.Active = j;
				n.FilteredTickets = 0;
			...
			jQuery.each(f.Groups, function(m, n) {
			...
			if (!f.Settings.TicketsListContainer && f.Settings.RowSelector && (l == null || l.length == 0)) {
				...
				l = [];
				var i = /<\S[^><]*>/g;
				jQuery(f.Settings.RowSelector).each(function() {
					var v = null;
					var m = null;
					var q = jQuery(this).find(f.Settings.SectionSelector).text().replace(i, "");
					if (q == "" && f.Settings.ZoneSelector) {
						v = jQuery(this).find(f.Settings.ZoneSelector).text().replace(i, "")
					} else {
						m = jQuery(this).find(f.Settings.RowNumberSelector).text().replace(i, "")
					}
					var o = (f.Settings.PriceSelector ? jQuery(this).find(f.Settings.PriceSelector).text().replace(i, "") : 0);
					var n = 0;
					if (f.Settings.QuantitySelector) {
						var p = jQuery(this).find(f.Settings.QuantitySelector);
						if (p.length > 0) {
							if (p.length == 1) {
								n = jQuery.trim(p.text().replace(i, ""))
							} else {
								n = [];
								jQuery.each(p, function(w, x) {
									n.push(parseInt(jQuery.trim(jQuery(x).text().replace(i, ""))))
								})
							}
						}
					}
					var r = (f.Settings.eTicketSelector ? jQuery(this).find(f.Settings.eTicketSelector).length > 0 : false);
					var s = jQuery(this);
					var u = "Zone";
					var t = null;
					if (v) {
			...
			jQuery.each(l, function(p, n) {
				...
				if (n.quantity) {
					if (typeof(n.quantity) == "object") {
						jQuery.each(n.quantity, function(r, q) {
				...
				if (o && (!(f.Selection.length > 0 && jQuery.inArray(n.SectionKey, f.Selection) == -1) || (n.ZoneKey && f.Groups[n.ZoneKey].Selected))) {
		...
		AddSectionMapping: function(g, h) {
			if (f.Sections[g]) {
				if (jQuery.inArray(h, f.Sections[g].Mappings) == -1) {
					f.Sections[g].Mappings.push(h)
				}
			}
			if (f.OtherSections[g]) {
				if (jQuery.inArray(h, f.OtherSections[g].Mappings) == -1) {
		...
		GetZone: function(h) {
			var g = "";
			jQuery.each(f.Groups, function(k, j) {
				if (j.Name.toLowerCase() == h.toLowerCase()) {
					g = j.Id;
					return false
				}
				jQuery.each(j.Alias, function(i, l) {
					if (j.Alias == h) {
						g = j.Id;
						return false
					}
		...
		GetSectionKey: function(h, k) {
			...
			SectionName = jQuery.trim(SectionName.replace(/[^0-9a-z]+/g, " "));
			var j = "";
			var m = "";
			var l = [];
			jQuery.each(f.Groups, function(o, n) {
				jQuery.each(n.Alias, function(p, r) {
					...
					if ((q == 0 || q == SectionName.length - r.length) && (r.length >= m.length)) {
						...
						if (n.SectionCount == 1) {
							...
							j = jQuery.trim(SectionName.replace(m, ""));
			...
			jQuery.each(f.Sections, function(p, o) {
				...
				if (o.Name.toLowerCase() == j) {
					if (!(n.IsExactMatch && jQuery.inArray(n.Id, l) == -1)) {
				...
				if (g == "Unmapped" && o.ExactMatchAlias != null && o.ExactMatchAlias.length > 0) {
					jQuery.each(o.ExactMatchAlias, function(q, r) {
						if (r == SectionName) {
							g = o.Key;
							return false
						}
				...
				if (g == "Unmapped" && o.Alias != null && o.Alias.length > 0) {
					jQuery.each(o.Alias, function(q, r) {
						if (r == SectionName) {
							g = o.Key;
							return false
						} else {
		...
		ResolveOtherSection: function(h) {
			var g = "Unmapped";
			jQuery.each(f.OtherSections, function(j, i) {
				if (i.IsExactMatch) {
					if (j.toLowerCase() == h) {
						g = j;
						return false
				...
				if (i.Alias != null && i.Alias.length > 0) {
					jQuery.each(i.Alias, function(k, l) {
						if (i.IsExactMatch) {
							if (l == h) {
								g = j;
								return false
		...
		IsTicketFiltered: function(g) {
			if (f.Settings.TicketsFilter.Ids != null && jQuery.inArray(g.id, f.Settings.TicketsFilter.Ids) == -1) {
				return false
			}
			...
			if (!isNaN(f.Settings.TicketsFilter.Quantity) && f.Settings.TicketsFilter.Quantity > 0) {
				if (f.Settings.TicketsFilter.Comparison == "eq") {
					if (typeof(g.quantity) == "object") {
						if (jQuery.inArray(f.Settings.TicketsFilter.Quantity, g.quantity) == -1) {
							return false
						}
					} else {
					...
					if (f.Settings.TicketsFilter.Comparison == "gt" && g.MaxQuantity > 0 && g.MaxQuantity < f.Settings.TicketsFilter.Quantity) {
						...
						if (f.Settings.TicketsFilter.Comparison == "bt") {
							if (typeof(g.quantity) == "object") {
								...
								if (jQuery.inArray(f.Settings.TicketsFilter.MaxQuantity, g.quantity) == -1) {
									return false
								}
							} else {
		...
		ShowTickets: function() {
			if (f.Settings.TicketsListContainer == null) {
				if (f.Settings.RowSelector != null) {
					jQuery(f.Settings.RowSelector).hide();
					jQuery.each(f.FilteredTickets, function(i, h) {
						h.RowObject.show()
					})
				}
				return
			...
			c.Initialize({
				...
				ColumnsFormatter: function(h) {
					...
					jQuery.each(f.Settings.Columns, function(i, j) {
				...
				RowFormatter: function(i, j) {
					...
					jQuery.each(f.Settings.Columns, function(k, l) {
						if (l.Visible == null || l.Visible) {
							...
							m.children(".BuyLink").click(function() {
								var o = jQuery(this).attr("lang");
								var n = 0;
								i.children("td").each(function() {
									var p = jQuery(this).children("[lang=Quantity]");
									if (p.length > 0) {
										n = jQuery(p[0]).val();
										return false
									}
							...
							m.children(".Notes").hover(function(p) {
								var n = jQuery.trim(jQuery(this).attr("title"));
								if (n != "") {
									var o = jQuery(this).position();
									f.Tooltip.css({
										top: o.top + 20,
										left: o.left + 20
									}).html(n);
								...
								jQuery(this).attr("title", f.Tooltip.html());
								f.Tooltip.hide()
							})
						}
					});
				...
				NoResultsFormatter: function(h) {
					return h.append(jQuery("<td>").attr("colspan", g).html(f.Settings.NoResultsFormatter()))
				}
			});
		...
		ShowTooltip: function(k, j, n) {
			...
			if (f.Settings.EnableTooltipSectionView) {
				var l = function() {
					...
					jQuery(".SectionViewBG").remove()
				};
				var g = function(p) {
					f.TooltipZoomed = true;
					jQuery("<div>").attr("class", "SectionViewBG").css("width", "100%").css("height", "100%").css("position", "fixed").css("left", 0).css("top", 0).css("z-index", 999).appendTo(jQuery("body")).click(function() {
						l()
					});
					var o = f.Tooltip.position();
					f.Tooltip.css("position", "fixed").css("opacity", 0).css("left", (jQuery(window).outerWidth(true) - f.SectionViewSize) / 2).css("top", (jQuery(window).outerHeight(true) - (f.SectionViewSize / 2)) / 2).animate({
						opacity: 1
					}, "slow");
					p.parent().css("width", f.SectionViewSize);
					p.css("height", Math.round(f.SectionViewSize / 2));
					p.attr("src", m.SectionViewUrl);
					jQuery(".SectionViewZoom", f.Tooltip).remove();
					p.unbind();
					p.css("cursor", "auto")
				};
				var i = (a.browser == "Firefox" ? "-moz-zoom-in" : (a.browser == "Chrome" || a.browser == "Safari" ? "-webkit-zoom-in" : "pointer"));
				jQuery("img", f.Tooltip).css("cursor", i).click(function(o) {
					o.preventDefault();
					g(jQuery(this))
				});
				jQuery(".SectionViewZoom", f.Tooltip).css("cursor", i).click(function(o) {
					o.preventDefault();
					g(jQuery("img", f.Tooltip))
				});
				jQuery(".SectionViewClose", f.Tooltip).click(function(o) {
					l()
				});
		...
		MoveTooltip: function(k, i) {
			...
			j.x += h.left;
			j.y += h.top;
			if (j.x + f.Tooltip.width() > h.left + f.Container.outerWidth(true) || j.x + f.Tooltip.width() > jQuery(window).width()) {
				g = "BottomLeft";
				j = k.GetCorner(i, g);
				j.y += h.top;
				j.x = h.left + j.x - f.Tooltip.width() - 10
			}
			if (j.y + f.Tooltip.height() > h.top + f.Container.outerHeight(true) || j.y + f.Tooltip.height() > jQuery(window).height()) {
				if (g == "BottomLeft") {
					g = "TopLeft"
				} else {
					g = "TopRight"
		...
		ShowSectionView: function(k, g) {
			if (f.Settings.SectionViewContainer && f.Settings.SectionViewTrigger == g) {
				...
				if (g == "click") {
					...
					var i = jQuery("<div>").html(i).append(jQuery("<div>").addClass("CloseButton").click(function() {
						f.HideSectionView(g)
					}));
					h.append(i)
				}
				h.append('<img src="' + j.SectionViewUrl + '" alt="' + i + '" style="max-width:100%;"/>');
				jQuery(f.Settings.SectionViewContainer).html(h);
				jQuery(f.Settings.SectionViewContainer).show()
			}
		},
		HideSectionView: function(g) {
			if (f.Settings.SectionViewContainer && f.Settings.SectionViewTrigger == g) {
				jQuery(f.Settings.SectionViewContainer).html("");
				jQuery(f.Settings.SectionViewContainer).hide()
			}
		},
				...
				case "Reset":
					if (!f.Settings.TicketsListContainer && f.Settings.RowSelector) {
						...
						if (f.Settings.Tickets) {
							jQuery.each(f.Settings.Tickets, function(h, i) {
								i.SectionKey = null
							})
						} else {
							f.Settings.Tickets = []
					...
					jQuery.extend(f.Settings, f.GetStyles(f.Settings.ColorScheme));
					jQuery(this).tuMap(f.Settings);
					break;
				case "ProcessTickets":
					f.ProcessTickets();
					f.SortTickets();
		...
		GenerateGroups: function() {
			var k = "";
			jQuery.each(f.Groups, function(p, o) {
			...
			jQuery(f.Settings.GroupsContainer).html(k).addClass("tuMapGroupsContainer");
			var n = 0;
			jQuery(f.Settings.GroupsContainer).children(".tuMapGroup").each(function() {
				if (jQuery(this).outerWidth(true) > n) {
					n = Math.round(jQuery(this).outerWidth(true))
				}
			});
			if (!f.InteractiveMode) {
				jQuery(f.Settings.GroupsContainer).removeClass("tuMapActiveGroupsContainer")
			} else {
				jQuery(f.Settings.GroupsContainer).addClass("tuMapActiveGroupsContainer");
				jQuery(f.Settings.GroupsContainer).children("div").mouseenter(function() {
					if (jQuery(this).hasClass("Disabled")) {
						...
						f.HighlightGroup(jQuery(this).attr("lang"))
					}
				}).mouseleave(function() {
					if (jQuery(this).hasClass("Disabled")) {
						f.Tooltip.hide()
					} else {
						f.ResetGroup(jQuery(this).attr("lang"))
					}
				}).click(function() {
					if (jQuery(this).hasClass("Disabled")) {
						return
			...
			var g = jQuery(f.Settings.GroupsContainer).outerWidth(true);
			var m = jQuery(f.Settings.GroupsContainer + " > DIV").length;
			var i = parseInt(g / n);
			...
			n = Math.floor(g / i);
			var j = 0;
			jQuery(f.Settings.GroupsContainer + " .tuMapGroup").each(function() {
				if (j == 0) {
					jQuery(f.Settings.GroupsContainer).append(jQuery("<div>").addClass("TempContainer").css("width", n))
				}
				jQuery(f.Settings.GroupsContainer + " > .TempContainer:last").append(jQuery(this));
				j++;
				if (j == l) {
					j = 0
				}
			});
			jQuery(f.Settings.GroupsContainer + " > .TempContainer").removeClass("TempContainer");
			jQuery(f.Settings.GroupsContainer).append('<br style="clear:both;"/>')
		},
		OnLoad: function(h, j) {
			try {
				if (j.Status == 1) {
					...
					e.Initialize(jQuery(this));
					if (j.Groups && j.Groups.length > 0) {
						jQuery.each(j.Groups, function(o, n) {
							n.ActiveSections = 0;
							f.Groups[n.Id] = n
						})
					}
					if (j.Sections != null) {
						...
						if (f.Settings.SectionViewContainer) {
							f.SectionViewSize = f.CalculateSVWidth(jQuery(f.Settings.SectionViewContainer).width())
						} else {
						...
						f.SectionCounts = {};
						f.Sections = {};
						jQuery.each(j.Sections, function(o, n) {
							f.InitSection(n, i)
						});
						var m = function() {
							e.Zoom = f.Settings.ZoomLevel;
					...
					if (j.OtherSections != null) {
						jQuery.each(j.OtherSections, function(o, n) {
					...
					if (f.Settings.FailoverMapUrl) {
						var g = jQuery("<img>").attr("src", f.Settings.FailoverMapUrl).attr("alt", "Venue Map").css("max-width", f.Container.outerWidth(true)).load(function() {
							f.OnLoadContinue(j)
						});
						jQuery("<div>").css("text-align", "center").append(g).appendTo(f.Container)
					}
				}
			} catch (l) {
		...
		OnLoadContinue: function(g) {
			...
			f.SortTickets();
			if (f.InteractiveMode) {
				jQuery.each(f.OtherSections, function(h, i) {
					if (i.Enabled) {
		...
		GenerateFilters: function() {
			if (f.Settings.TicketsListContainer) {
				...
				jQuery(f.Settings.TicketsListContainer).css("overflow", "visible");
				jQuery(f.Settings.TicketsListContainer).html("");
				var h = jQuery(f.Settings.TicketsListContainer).outerHeight(true);
				var k = 0;
				if (f.PreferredFlag || f.eTicketFlag) {
					var i = jQuery("<div>").addClass("tuMapPreferredFilter");
					if (f.PreferredFlag) {
						var l = jQuery("<div>").addClass("tuMapPreferred");
						l.append(jQuery("<span>").addClass("Icon"));
						var g = jQuery("<input>").attr("type", "checkbox").attr("checked", (f.Settings.PreferredFirst ? 'checked="checked"' : ""));
						g.change(function(m) {
							f.Settings.PreferredFirst = jQuery(this).is(":checked");
							f.SortTickets();
							f.ShowTickets()
						});
						l.append(g);
						l.append(jQuery("<span>").html(f.Settings.PreferredText));
						i.append(l)
					}
					if (f.eTicketFlag) {
						var j = jQuery("<div>").addClass("tuMapeTicket");
						j.append(jQuery("<span>").addClass("Icon"));
						j.append(jQuery("<span>").html("E-Ticket"));
						i.append(j)
					}
					jQuery(f.Settings.TicketsListContainer).append(i);
					jQuery("<br>").css("clear", "both").appendTo(i);
					if (h > 0) {
						k = i.outerHeight(true)
					}
				}
				f.vTableContainer = jQuery("<div>");
				jQuery(f.Settings.TicketsListContainer).append(f.vTableContainer);
				if (h == 0) {
					h = f.Container.outerHeight(true);
					if (f.Settings.GroupsContainer) {
						h += jQuery(f.Settings.GroupsContainer).outerHeight(true)
					}
					h = h - Math.abs(f.Container.offset().top - jQuery(f.vTableContainer).offset().top)
				}
				h = h - k;
		...
		Initialize: function(h) {
			...
			if (h.ColorScheme != null) {
				jQuery.extend(f.defaults, f.GetStyles(h.ColorScheme))
			}
			f.Settings = jQuery.extend({}, f.defaults, h);
			f.Container = jQuery(this);
			f.Container.unbind();
			jQuery(".tuMapTooltip").remove();
			var g = f.Container.position();
			f.Tooltip = jQuery('<div class="tuMapTooltip"></div>').insertAfter(f.Container).css({
				position: "absolute",
				display: "none",
				"z-index": 20000,
				left: g.left,
			...
			jQuery.ajax({
				...
				success: function(j) {
					if (f.Settings.GroupsContainer != null) {
						jQuery(f.Settings.GroupsContainer).html("")
					}
					if (f.Settings.TicketsListContainer != null) {
						jQuery(f.Settings.TicketsListContainer).html("")
					}
	...
	var c = {
		defaults: {
			...
			ColumnsFormatter: function(g) {
				return g.append(jQuery("<th>").text("Column"))
			},
			RowFormatter: function(h, g) {
				return h.attr("lang", g).append(jQuery("<td>").text("Row" + g))
			},
		...
		Initialize: function(i) {
			...
			c.RowData = [];
			c.RowHeight = 0;
			c.Settings = jQuery.extend({}, c.defaults, i);
			c.Settings.Container.unbind();
			c.Settings.Container.html("").attr("class", "tuMapTicketList");
			c.ColTable = c.Settings.TableFormatter(jQuery("<table>").attr("cellpadding", 3).attr("cellspacing", 0).attr("style", "width:100%;"));
			c.ColTable.append(jQuery("<thead>").append(c.Settings.ColumnsFormatter(jQuery("<tr>"))));
			c.ColTable.appendTo(c.Settings.Container);
			c.TableContainer = jQuery("<div>").css("overflow", "auto").css("height", c.Settings.Container.height() - c.ColTable.height() - 1);
			...
			if (c.Settings.Rows == 0) {
				c.Body.append(c.Settings.NoResultsFormatter(jQuery("<tr>")));
				c.AdjustColumns();
				return
			}
			for (var g = 0; g < c.Settings.Rows; g++) {
				...
				if (c.Table.height() > c.TableContainer.height()) {
					c.RowHeight = jQuery(c.Table.find("tr")[1]).height();
					var h = jQuery("<tr>").attr("height", c.Settings.Rows * c.RowHeight);
					c.Body.append(h);
					c.Fillers.push({
						startIndex: g + 1,
		...
		CreateRow: function(u, p) {
			...
			for (var o = k.length; o--;) {
				...
				for (var r = q; r <= t; r++) {
					var s = w[r];
					s.tr = jQuery("<tr>").height(g + "px").insertBefore(n.tr)
				}
				var m = false;
				var l = n.startIndex;
				if (t < n.endIndex) {
		...
		AdjustColumns: function() {
			...
			for (var g = 0; g < i.length - 1; g++) {
				jQuery(h[g]).width(jQuery(i[g]).width())
			}
			c.Table.css("margin-top", -jQuery(i[0]).outerHeight(true) - 1)
		}
	};
	var e = {
		TileSize: 100,
		...
		OnReset: function() {
			jQuery.each(f.Selection, function(h, g) {
				if (f.Sections[g]) {
			...
			f.Selection = [];
			f.ZoneSelected = false;
			jQuery.each(f.Groups, function(g, h) {
			...
			jQuery(".tuMapControlSelected").each(function() {
				jQuery(this).removeClass("tuMapControlSelected")
			});
			jQuery(".tuMapControlBlink").each(function() {
				jQuery(this).removeClass("tuMapControlBlink")
			});
		...
		Initialize: function(i) {
			...
			e.Container = jQuery("<div>").attr("class", "tuMapContainer").attr("style", "position:relative;margin:auto;overflow:hidden;width:" + h + "px;height:" + n + "px").appendTo(i).disableSelection().mousemove(function(o) {
				if (e.MouseDown) {
					if (e.CurrentShape) {
						e.Wrapper.trigger("OnMouseOut", [e.GetAbsolutePosition(o), e.CurrentShape])
					}
					jQuery(e.Canvas).css("cursor", "move");
					e.CurrentShape = null;
			...
			e.Wrapper = jQuery("<div>").attr("style", "z-index:0;position:absolute;width:" + e.Side + "px;height:" + e.Side + "px").disableSelection().appendTo(e.Container).mousedown(function(o) {
				e.MouseDown = true;
				...
				e.MouseDown = false;
				e.Dragging = false;
				jQuery(e.Canvas).css("cursor", "auto")
			...
			e.GenerateTiles();
			var g = jQuery("<canvas />").attr("style", "position:absolute; top:0; left:0; z-index:1").disableSelection().appendTo(e.Wrapper);
			e.Canvas = g[0];
			...
			e.ZoomControls = jQuery("<div>").addClass("ZoomControls").appendTo(e.Container);
			jQuery("<div>").addClass("ZoomIn").appendTo(e.ZoomControls).click(function() {
				e.ZoomIn()
			});
			jQuery("<div>").addClass("ZoomOut").appendTo(e.ZoomControls).click(function() {
				e.ZoomOut()
			});
			e.ResetContainer = jQuery("<div>").attr("style", "position:absolute;right:5px;top:5px;").appendTo(e.Container);
			e.ResetButton = jQuery("<div>").addClass("tuMapControl").html(f.Settings.ResetButtonText).appendTo(e.ResetContainer).click(function() {
				f.Reset()
			});
			jQuery(".tuMapControlsContainer").remove();
			if (f.Settings.ControlsPosition == "Inside") {
				e.ControlsContainer = jQuery('<div class="tuMapControlsContainer">').attr("style", "text-align:center;bottom:0;left:50%;position:absolute;").appendTo(e.Container)
			} else {
				e.ControlsContainer = jQuery('<div class="tuMapControlsContainer">').attr("style", "text-align:center;margin:0 auto;").insertAfter(i)
			}
			if (e.IsOldIE()) {
		...
		AddMapControl: function(i, g, h) {
			jQuery("<div>").addClass("tuMapControl").html(i).appendTo(e.ControlsContainer).click(function() {
				var j = true;
				if (h) {
					if (jQuery(this).hasClass("tuMapControlSelected") || jQuery(this).hasClass("tuMapControlBlink")) {
						jQuery(this).removeClass("tuMapControlSelected");
						jQuery(this).removeClass("tuMapControlBlink");
						j = false
					} else {
						jQuery(this).addClass("tuMapControlSelected")
					}
				}
				g(j)
			});
		...
		GenerateTiles: function() {
			...
			for (var h = 0; h < g; h++) {
				for (var k = 0; k < g; k++) {
					...
					jQuery(i).attr("style", "position:absolute;top:" + (k * e.TileSize) + "px;left:" + (h * e.TileSize) + "px;z-index:0;width:" + e.TileSize + "px;height:" + e.TileSize + "px;").appendTo(e.Wrapper).disableSelection();
					this.Tiles.push(jQuery(i))
				}
			}
		},
		...
		TriggerEvent: function(g, h) {
			if (h) {
				if (e.CurrentShape == null) {
					jQuery(e.Canvas).css("cursor", "pointer");
	...
	b.Map.Section = function(g) {
		...
		this.MaxPriceValue = "";
		this.Style = {};
		jQuery.extend(this.Style, (this.Active ? f.Settings.NormalStyle : f.Settings.InactiveStyle));
		this.Selected = false;
		this.Mappings = [];
		this.Coords = [];
		this.Markers = [];
		var h = this;
		jQuery.each(this.Coordinates, function(j, i) {
		...
		this.GetCenter = function() {
			var i = 0,
				k = 0,
				j = 0;
			jQuery.each(this.Coords, function(l, m) {
				j++;
				i += m.x * e.ZoomLevel;
				k += m.y * e.ZoomLevel
			});
		...
		this.GetCorner = function(r, i) {
			...
			var q = 10000,
				...
				p = 10000,
				m = 0;
			jQuery.each(this.Coords, function(v, w) {
				if (w.x < q) {
					q = w.x
				} else {
					if (w.x > o) {
		...
		this.DrawMarkers = function() {
			jQuery.each(this.Markers, function(j, i) {
	...
	jQuery.fn.tuMap = function(g) {
		if (f[g] != null) {
			...
			if (typeof g === "object" || !g) {
				...
				jQuery.error("Method " + g + " does not exist")
			}
		}
	};
	var a = {
